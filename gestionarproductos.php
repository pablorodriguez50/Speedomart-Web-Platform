...
<body class="cbp-spmenu-push">
            ...
            <div class="navbar-collapse">
				<nav class="cbp-spmenu cbp-spmenu-vertical cbp-spmenu-left" id="cbp-spmenu-s1">
					<ul class="nav" id="side-menu">
						...
						<li>
							<a href="gestionarproductos.php" class="active"><i class="fa fa-tag nav_icon"></i>Gestionar Productos<span class="fa arrow"></span></a>
						</li>
						<li>
   ...
   // Codigo INICIO
    include ("conex.php");
//idSup es un campo del empleado con sesion iniciada
	$query="SELECT producto.nombre, producto.descripcion, producto.precio, producto.codigo_barra, producto.imagen, stock_prod.stock, categoria.nombreCAT 
			FROM producto INNER JOIN stock_prod ON stock_prod.idSup = $idSup AND producto.stock = stock_prod.idStock_prod 
...
//Tabla Productos
	...
	echo "<h3 class='title1'>Listado de Productos :</h3>";
    echo "<div class='panel-body widget-shadow'>";

	echo "<table class='table'>";

		echo "<button onClick='window.location=\"agregarproducto.php\";'/ class='btn btn-success add-prod'>
								<span class='fa fa-plus'></span>Agregar Producto </button>";
		
		echo "<thead><tr><th>Codigo de Barra</th><th>Nombre</th><th>Descripcion</th><th class='action'>Accion</th></thead>";
		...
		{   		
				echo "<tbody>";
				echo "<tr title='informacion detallada' class='selector-producto' >";
				echo "<td class='selector' data-toggle='modal' data-target='#infodetallada".$fila['codigo_barra']."' >".$fila["codigo_barra"]."</td>";
				echo "<td class='selector' data-toggle='modal' data-target='#infodetallada".$fila['codigo_barra']."' >".$fila["nombre"]."</td>";
				echo "<td class='selector' data-toggle='modal' data-target='#infodetallada".$fila['codigo_barra']."' >".$fila["descripcion"]."</td>";
				echo "<td class='buttons-admin'><button onClick='window.location=\"modificarproducto.php?codigo_barra=".$fila["codigo_barra"]."\";'/ title='modificar producto' class='btn btn-primary btn-admin1'><span class='fa fa-refresh'></span></button>
				</button><button  onClick='window.location=\"productosinstock.php?codigo_barra=".$fila["codigo_barra"]."\";'/ title='sin stock' class='btn btn-danger btn-admin2'>
				<span class='fa fa-ban'></span></td>";
				echo "</tr>";
				echo "</tbody>";
           ...
           //modal
            ...
            echo "<div class='modal fade' id='infodetallada".$fila['codigo_barra']."' role='dialog'>";
			...
			echo  "<b>Codigo de barra : </b>" .$fila["codigo_barra"]."<br>";
			echo  "<b>Nombre : </b>" .$fila["nombre"]."<br>";
			echo  "<img src='".$fila["imagen"]."' class='img-prod'>";
			echo  "<b>Descripcion : </b>" .$fila["descripcion"]."<br>";
	      	echo  "<b>Precio : </b>" .$fila["precio"]."<br>";
			echo  "<b>Stock : </b>" .$fila["stock"]."<br>";
			echo  "<b>Categoria : </b>" .$fila["nombreCAT"]."<br>";
			echo "</div>";
			echo "<div class='modal-footer'>";
			echo "<button onClick='window.location=\"modificarproducto.php?codigo_barra=".$fila["codigo_barra"]."\";'/  class='btn btn-primary'><span class='fa fa-refresh'></span> Modificar</button>";
			echo "<button onClick='window.location=\"productosinstock.php?codigo_barra=".$fila["codigo_barra"]."\";'/  class='btn btn-danger'><span class='fa fa-ban'></span> Sin Stock </button>";
